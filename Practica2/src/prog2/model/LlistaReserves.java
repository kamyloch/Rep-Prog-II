...
public class LlistaReserves implements InLlistaReserves {
    ...
    public void afegirReserva(Allotjament allotjament, Client client, LocalDate entrada, LocalDate sortida) throws ExcepcioCamping {
        ...
        if (!isEstadaMinima(allotjament, entrada, sortida))
            throw new ExcepcioCamping("Les dates sol·licitades pel client " + client.getNom() + " amb DNI: " + client.getDni() + " no compleixen l'estada mínima per l'allotjament amb identificador " + allotjament.getId());

        if(!allotjamentDisponible(allotjament, entrada, sortida))
            throw new ExcepcioCamping("L’allotjament amb identificador " + allotjament.getId() + " no està disponible a la data demanada " + entrada.toString() + " a " +sortida.toString()  + " pel client " + client.getNom() + " amb DNI: " + client.getDni());
